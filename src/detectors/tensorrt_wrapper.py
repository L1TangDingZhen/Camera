...
class TensorRTRTMPose:
    ...
    def postprocess(self, outputs: dict, bbox: np.ndarray, img_shape: Tuple[int, int]) -> np.ndarray:
        ...
        # Get argmax for each keypoint
        x_coords = np.argmax(simcc_x[0], axis=-1)  # (17,)
        y_coords = np.argmax(simcc_y[0], axis=-1)  # (17,)

        # Get confidence scores (max probability)
        x_scores = np.max(simcc_x[0], axis=-1)
        y_scores = np.max(simcc_y[0], axis=-1)
        scores = np.sqrt(x_scores * y_scores)  # Geometric mean

        # Convert from SimCC coordinates to pixel coordinates
        ...
        x_pixels = x_coords * x_scale
        y_pixels = y_coords * y_scale
